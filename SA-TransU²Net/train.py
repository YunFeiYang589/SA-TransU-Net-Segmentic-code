...
from src.model import u2net_swin
from src.train_utils import train_one_epoch, evaluate, get_params_groups, create_lr_scheduler
from my_dataset import DUTSDataset
import transforms as T
from src.attention_modules import SpatialAttention
import warnings
...
class SODPresetTrain:
    def __init__(self, base_size: Union[int, List[int]], crop_size: int,
                 hflip_prob=0.5, mean=(0.546, 0.510, 0.489), std=(0.316, 0.312, 0.295)):
...
class SODPresetEval:
    def __init__(self, base_size: Union[int, List[int]],  mean=(0.546, 0.510, 0.489), std=(0.316, 0.312, 0.295)):
        self.transforms = T.Compose([
            T.ToTensor(),
            T.Resize(base_size, resize_mask=True),  # 关键修改：设置为True
            T.Normalize(mean=mean, std=std),
        ])
...
def main(args):
    ...
    model = u2net_swin(use_attention=True)
    model.to(device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        mean_loss, lr = train_one_epoch(
            ...
            scaler=scaler, use_boundary_loss=True, boundary_weight=0.5
        )
